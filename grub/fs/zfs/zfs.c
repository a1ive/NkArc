...
zfs_fetch_nvlist(struct grub_zfs_device_desc* diskdesc, char** nvlist)
{
	...
	*nvlist = grub_malloc(VDEV_PHYS_SIZE);
	...
	err = grub_disk_read(diskdesc->dev, diskdesc->vdev_phys_sector, 0,
...
static grub_err_t
scan_disk(grub_disk_t dev, struct grub_zfs_data* data,
	int original, int* inserted)
{
	...
	if (grub_disk_native_sectors(dev) == GRUB_DISK_SIZE_UNKNOWN)
	...
	for (label = 0; ubbest == NULL && label < vdevnum; label++)
	{
		desc.vdev_phys_sector
			...
			+ (label < VDEV_LABELS / 2 ? 0 :
				ALIGN_DOWN(grub_disk_native_sectors(dev), sizeof(vdev_label_t))
				- VDEV_LABELS * (sizeof(vdev_label_t) >> SPA_MINBLOCKSHIFT));

		/* Read in the uberblock ring (128K). */
		err = grub_disk_read(dev, desc.vdev_phys_sector
...
{
	grub_disk_iterate(scan_devices_iter, data);
	return GRUB_ERR_NONE;
}
	...
	grub_size_t len, void* buf)
{
	...
	case DEVICE_LEAF:
	{
		...
		return grub_disk_read(desc->dev, sector, 0, len, buf);
	}
	case DEVICE_MIRROR:
	{
		grub_err_t err = GRUB_ERR_NONE;
...
grub_err_t
grub_zfs_fetch_nvlist(grub_disk_t dev, char** nvlist)
{
...
static grub_err_t
zfs_label(grub_disk_t disk, char** label)
{
	char* nvlist;
	grub_err_t err;
	struct grub_zfs_data* data;
...
static grub_err_t
zfs_uuid(grub_disk_t disk, char** uuid)
{
	struct grub_zfs_data* data;
...
static grub_err_t
zfs_mtime(grub_disk_t disk, grub_int64_t* mt)
{
	struct grub_zfs_data* data;
	grub_zfs_endian_t ub_endian = GRUB_ZFS_UNKNOWN_ENDIAN;
	uberblock_t* ub;
...
grub_err_t
grub_zfs_getmdnobj(grub_disk_t dev, const char* fsfilename,
...
static grub_err_t
grub_zfs_dir(grub_disk_t disk, const char* path,
